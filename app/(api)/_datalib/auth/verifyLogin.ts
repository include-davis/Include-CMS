 ...
 */
export async function comparePasswordHash(providedPassword: string, storedHash: string): Promise<boolean> {
    return bcrypt.compare(providedPassword, storedHash);
}

export async function verifyLogin(body: {email: string; password: string}){
    try {
        ...
        const isMatch = await comparePasswordHash(user.password, user.passwordHash);
        if (!isMatch) {
            return { ok: false, error: 'AuthenticationError' };
        }
